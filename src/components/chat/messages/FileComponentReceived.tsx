...
export const FileComponentReceived = ({ files, isFirst }: Props) => {
  return (
    <>
      <span
        className={
          isFirst
            ? 'p-1 rounded-lg inline-block bg-gray-300'
            : 'p-1 rounded-lg rounded-tl-none inline-block bg-gray-300'
        }
      >
        {files.map((file) => {
          return (
              ...
              <div key={file.path}>
                ...
                !file.path.toLowerCase().endsWith('.wav') ? (
                  <a
                    ...
                    download
                    className="inline-block px-4 py-2 text-sm font-medium text-black hover:bg-gray-400 transition duration-300 ease-in-out rounded-md shadow-lg w-52"
                  >
                    <svg
                      ...
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      className="-ml-1 mr-2 h-5 w-5 text-black transform rotate-180 float-left"
                    >
                      <path
                        strokeLinecap="round"
